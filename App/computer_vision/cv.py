...
class VisionProcessor:
    ...
    def __init__(self, client, record=False, thresh=240, min_area=50, 
        ...
        # Debug visualization
        self.show_debug_masks = False  # Toggle with 'd' key
        
        # Calibration sync
        self.calibration_ready = False  # Signal that vision is ready for calibration
    ...
    def _apply_roi_mask(self, mask):
        ...
        IMPORTANT: ROI is DISABLED during calibration mode!
        """
        # NEVER use ROI in calibration mode - need full frame detection
    ...
    def handler(self, _ch, data):
        ...
        if len(data) == 1:
            # In calibration mode, LED index is set manually by calibrator
            # Don't let BLE notifications overwrite it!
    ...
    async def start(self):
        ...
        if self.calibration_mode:
            print("🎬 VISION - CALIBRATION MODE (Full Frame Detection)", file=sys.stderr, flush=True)
        else:
        ...
        if not self.calibration_mode:
            ...
            print("🔧 Calibration mode - waiting for manual LED control", file=sys.stderr, flush=True)
        
        prev_time = time.time()
        
        # Signal that vision is ready (important for calibration!)
        self.calibration_ready = True
        print("✅ Vision loop ready", file=sys.stderr, flush=True)
        ...
        try:
            while True:
                ...
                # Debug: Show current_led at frame start (calibration mode only)
                if self.calibration_mode and self.stats['total'] == 1:
                    print(f"[CV] First frame - current_led is: {self.current_led}")
                
                # === ROI UPDATE ===
                # Skip ROI updates in calibration mode - need full frame detection
                if not self.calibration_mode:
                ...
                # Track if using ROI this frame (never in calibration mode)
                using_roi = self.roi_enabled and self.roi_bbox is not None and not self.calibration_mode
                if using_roi:
                ...
                if candidates:
                    ...
                    # More lenient quality threshold in calibration mode
                    quality_threshold = 0.3 if self.calibration_mode else 0.5
                    
                    if best["quality"] < quality_threshold:
                        reason = f"Q={best['quality']:.2f}"
                        self.stats['rejected_quality'] += 1
                    else:
                        # Skip temporal validation in calibration mode - we just need ONE detection
                        if self.calibration_mode:
                            accepted = best
                ...
                if self.calibration_mode:
                    if self.stats['total'] % 20 == 0:  # Every 20 frames
                        print(f"[CV] Frame {self.stats['total']:04d} | LED:{self.current_led:2d} | " +
                              f"Candidates:{len(candidates)} | " +
                              (f"ACCEPTED: {blob_centers[0]}" if blob_centers else f"REJECTED: {reason}"))
                ...
                if self.calibration_mode:
                    cv2.putText(display, "CALIBRATION MODE", (10, 90),
                               cv2.FONT_HERSHEY_SIMPLEX, 1.0, (255, 0, 255), 3)
                    y_offset = 140
                else:
                    # ROI status (only shown in normal mode)
                ...
                # FPS
                cv2.putText(display, f"FPS:{fps:.1f}", (10, y_offset),
                ...
                if self.stats['total'] > 10:
                    ...
                    cv2.putText(display, f"Accept:{rate:.0f}%", (10, y_offset + 50),
                ...
                # Debug masks (toggle with 'd')
                if self.show_debug_masks and not self.calibration_mode:
                    cv2.imshow("1. All Bright Pixels", current_bright_clean)
                    cv2.imshow("2. NEW Bright (Change Detection)", new_bright)
                    
                    # Optional: Show ROI mask
                    if using_roi:
                        roi_mask = np.zeros((frame_height, frame_width), dtype=np.uint8)
                        x, y, w, h = self.roi_bbox
                        roi_mask[y:y+h, x:x+w] = 255
                        cv2.imshow("3. ROI Mask", roi_mask)
                ...
                if key == ord('q'):
                    ...
                    print(f"🔬 Debug masks: {'ON' if self.show_debug_masks else 'OFF'}")
                    if not self.show_debug_masks:
                        # Close debug windows
                        cv2.destroyWindow("1. All Bright Pixels")
                        cv2.destroyWindow("2. NEW Bright (Change Detection)")
                        cv2.destroyWindow("3. ROI Mask")
    ...
    def get_status(self):
        """Return current vision processor status (for debugging)."""
        return {
